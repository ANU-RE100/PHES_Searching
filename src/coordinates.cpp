...
ArrayCoordinate ArrayCoordinate_init(int row, int col,
                                     GeographicCoordinate origin)  // Ditch at some point?
{
...
double find_distance_sqd(GeographicCoordinate c1, GeographicCoordinate c2) {
  return (SQ(c2.lat - c1.lat) + SQ((c2.lon - c1.lon) * COS(RADIANS(0.5 * (c1.lat + c2.lat))))) *
         SQ(3600 * resolution * 0.001);
}
...
ArrayCoordinate convert_coordinates(GeographicCoordinate c, GeographicCoordinate origin) {
  return ArrayCoordinate_init(convert_to_int((origin.lat - c.lat) * 3600 - 0.5),
                              convert_to_int((c.lon - origin.lon) * 3600 - 0.5), origin);
}

ArrayCoordinate convert_coordinates(GeographicCoordinate c, GeographicCoordinate origin,
                                    double lat_res, double lon_res) {
  return ArrayCoordinate_init(convert_to_int((c.lat - origin.lat) / lat_res - 0.5),
                              convert_to_int((c.lon - origin.lon) / lon_res - 0.5), origin);
}

GeographicCoordinate convert_coordinates(ArrayCoordinate c) {
  return GeographicCoordinate_init(c.origin.lat - (c.row + 0.5) / 3600.0,
                                   c.origin.lon + (c.col + 0.5) / 3600.0);
}
