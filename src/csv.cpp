...
vector<RoughReservoir> read_rough_reservoir_data(char* filename)
{
    ...
    while (getline(inputFile, s)) {
        ...
        for (uint i=0; i<dam_wall_heights.size();i++) reservoir.areas.push_back(stod(line[6+dam_wall_heights.size()+i]));	
		for (uint i=0; i<dam_wall_heights.size();i++) reservoir.dam_volumes.push_back(stod(line[6+2*dam_wall_heights.size()+i]));
		reservoir.max_dam_height = stod(line[4]);
		reservoir.watershed_area = stod(line[5]);
		reservoir.identifier = line[0];
		for (uint ih=0; ih< dam_wall_heights.size(); ih++) {
			for (uint idir=0;idir < directions.size(); idir++){
				reservoir.shape_bound[ih][idir].row = stoi(line[6+3*dam_wall_heights.size()+(ih*directions.size()+idir)*2]);
				reservoir.shape_bound[ih][idir].col = stoi(line[6+3*dam_wall_heights.size()+1+(ih*directions.size()+idir)*2]);
...
void write_rough_pair_csv_header(FILE *csv_file)
{
	...
	"Upper Identifier","Upper latitude","Upper longitude","Upper elevation (m)","Upper dam height (m)","Upper max dam height (m)", "Upper water to rock estimate", "Upper area estimate (ha)",
	"Lower Identifier","Lower latitude","Lower longitude","Lower elevation (m)","Lower dam height (m)","Lower max dam height (m)","Lower water to rock estimate", "Lower area estimate (ha)",
...
{
	vector<string> header = {"Pair Identifier",
	"Upper Identifier","Upper latitude","Upper longitude","Upper elevation (m)","Upper dam height (m)","Upper max dam height (m)","Upper water to rock estimate", "Upper area estimate (ha)", "Upper Brownfield",
	"Lower Identifier","Lower latitude","Lower longitude","Lower elevation (m)","Lower dam height (m)","Lower max dam height (m)","Lower water to rock estimate", "Lower area estimate (ha)", "Lower Brownfield", "Lower Ocean",
...
void write_rough_pair_csv(FILE *csv_file, Pair *pair)
{
	...
	pair->upper.identifier, dtos(pair->upper.latitude,4), dtos(pair->upper.longitude,4), to_string(pair->upper.elevation), dtos(pair->upper.dam_height,1), dtos(pair->upper.max_dam_height,0), dtos(pair->upper.water_rock,1), dtos(pair->upper.area,1),
	pair->lower.identifier, dtos(pair->lower.latitude,4), dtos(pair->lower.longitude,4), to_string(pair->lower.elevation), dtos(pair->lower.dam_height,1), dtos(pair->lower.max_dam_height,0), dtos(pair->lower.water_rock,1), dtos(pair->lower.area,1),
...
vector<vector<Pair> > read_rough_pair_data(char* filename)
{
    ...
    while (getline(inputFile, s)) {
    	...
    	pair.lower = Reservoir_init(convert_coordinates(gc, origin), stoi(line[13]));
