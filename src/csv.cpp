...
vector<ExistingReservoir> read_existing_reservoir_data(char *filename) {
  ...
  while (getline(inputFile, s)) {
    ...
    ExistingReservoir reservoir =
        ExistingReservoir_init(line[0], stod(line[1]), stod(line[2]), stod(line[3]), stod(line[4]));
...
void write_rough_reservoir_csv_header(FILE *csv_file) {
  vector<string> header = {"Identifier",    "Latitude",           "Longitude",
                           "Elevation (m)", "Maximum dam height", "Watershed area (ha)"};
...
vector<RoughReservoir> read_rough_reservoir_data(char *filename) {
  ...
  while (getline(inputFile, s)) {
    ...
    GeographicCoordinate origin =
        get_origin(GridSquare_init((int)FLOOR(gc.lat) - EPS, (int)FLOOR(gc.lon) + EPS), border);
    ...
    for (uint i = 0; i < dam_wall_heights.size(); i++)
      reservoir.dam_volumes.push_back(stod(line[6 + 2 * dam_wall_heights.size() + i]));
    reservoir.max_dam_height = stod(line[4]);
    reservoir.watershed_area = stod(line[5]);
    reservoir.identifier = line[0];
    for (uint ih = 0; ih < dam_wall_heights.size(); ih++) {
      for (uint idir = 0; idir < directions.size(); idir++) {
        reservoir.shape_bound[ih][idir].row =
            stoi(line[6 + 3 * dam_wall_heights.size() + (ih * directions.size() + idir) * 2]);
        reservoir.shape_bound[ih][idir].col =
            stoi(line[6 + 3 * dam_wall_heights.size() + 1 + (ih * directions.size() + idir) * 2]);
        reservoir.shape_bound[ih][idir].origin = origin;
      }
    }

    reservoir.brownfield = stoi(line[6 + 3 * dam_wall_heights.size() +
                                     (dam_wall_heights.size() * directions.size()) * 2]) > 0;
    reservoir.pit = stoi(line[6 + 3 * dam_wall_heights.size() +
                              (dam_wall_heights.size() * directions.size()) * 2]) > 1;
    reservoir.ocean = stoi(line[6 + 3 * dam_wall_heights.size() +
...
vector<vector<Pair>> read_rough_pair_data(char *filename) {
  ...
  while (getline(inputFile, s)) {
    ...
    GeographicCoordinate origin = get_origin(
        GridSquare_init(convert_to_int(FLOOR(gc.lat + EPS)), convert_to_int(FLOOR(gc.lon + EPS))),
        border);
...
void write_pair_csv(FILE *csv_file, Pair *pair, bool output_FOM) {
  vector<string> line = {
      ...
      (pair->upper.brownfield && !pair->upper.pit ? "NA" : dtos(pair->upper.dam_height, 1)),
      ...
      dtos(pair->lower.latitude, 4),
      dtos(pair->lower.longitude, 4),
      ((pair->lower.brownfield || pair->lower.ocean) ? "NA" : dtos(pair->lower.area, 0)),
      dtos(pair->lower.volume, 1),
      (((pair->lower.brownfield && !pair->lower.pit) || pair->lower.ocean)
           ? "NA"
           : dtos(pair->lower.dam_height, 1)),
      ((pair->lower.brownfield || pair->lower.ocean) ? "NA" : dtos(pair->lower.dam_length, 0)),
      ((pair->lower.brownfield || pair->lower.ocean) ? "NA" : dtos(pair->lower.dam_volume, 2)),
      ((pair->lower.brownfield || pair->lower.ocean) ? "NA" : dtos(pair->lower.water_rock, 1)),
      pair->lower.country};
  if (output_FOM)
