  ...
  if (SHP != NULL) {
    ...
    for (uint i = 0; i < relevant_polygons.size(); i++) {
      for (int row = 0; row < filter->nrows(); row++) {
        vector<double> polygon_intersections =
            find_polygon_intersections(row, relevant_polygons[i], filter);
        ...
        for (uint j = 0; j < polygon_intersections.size() / 2; j++)
          for (int col = polygon_intersections[2 * j]; col < polygon_intersections[2 * j + 1];
               col++)
...
int main(int nargs, char **argv) {
    ...
    for (int col = 0; col < extent->ncols(); col++)
      if (extent->get(row, col)) {
        ...
        area_at_elevation[elevation_above_pp + 1] +=
            find_area(ArrayCoordinate_init(row, col, DEM->get_origin()));
      }
  ...
  printf(convert_string("Volume finding finished for " + str(square_coordinate) +
                        ". Runtime: %.2f sec\n"),
