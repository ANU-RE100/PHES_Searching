...
string get_html(Reservoir *reservoir, Pair *pair) {
  if (reservoir->pit) {
    return "              <html>\n"
           ...
           "              <table "
           "style=\"font-family:Arial,Verdana,Times;font-size:12px;text-align:left;width:100%;"
           "border-collapse:collapse;padding:3px 3px 3px 3px\">\n"
           "              <tr "
           "style=\"text-align:center;font-weight:bold;background:#9CBCE2\"><td>" +
           reservoir->identifier +
           "</td></tr>\n"
           "              <tr><td>\n"
           "              <table "
           "style=\"font-family:Arial,Verdana,Times;font-size:12px;text-align:left;width:100%;"
           "border-spacing:0px; padding:3px 3px 3px 3px\">\n"
           "              <tr><td>Class</td><td>" +
           ...
           "</td></tr>\n"
           "              <tr><td>Depth</td><td>" +
           dtos(reservoir->dam_height, 0) +
           "</td></tr>\n"
           "              <tr bgcolor=\"#D4E4F3\"><td>Latitude</td><td>" +
    ...
    return "              <html>\n"
           ...
           dtos(reservoir->longitude, 4) +
           "</td></tr>\n"
           "              <tr><td>Volume (GL)</td><td>" +
           dtos(reservoir->volume, 1) +
           "</td></tr>\n"
           "              <tr bgcolor=\"#D4E4F3\"><td>Country</td><td>" +
           reservoir->country +
           "</td></tr>\n"
           "              </table></td></tr></table></body></html>\n";
  } else {
...
string get_html(Pair *pair) {
  return "              <html>\n"
         ...
         string(1, pair->category) +
         "</td></tr>\n"
         "              <tr bgcolor=\"#D4E4F3\"><td>Head (m)</td><td>" +
         to_string(pair->head) +
         "</td></tr>\n"
         "              <tr><td>Separation (km)</td><td>" +
         dtos(pair->distance, 1) +
         "</td></tr>\n"
         "              <tr bgcolor=\"#D4E4F3\"><td>Average Slope (%)</td><td>" +
         dtos(pair->slope * 100, 0) +
         "</td></tr>\n"
         "              <tr><td>Volume (GL)</td><td>" +
         dtos(pair->volume, 1) +
         "</td></tr>\n"
         "              <tr bgcolor=\"#D4E4F3\"><td>Water to Rock (Pair)</td><td>" +
         dtos(pair->water_rock, 1) +
         "</td></tr>\n"
         "              <tr><td>Energy (GWh)</td><td>" +
         energy_capacity_to_string(pair->energy_capacity) +
         "</td></tr>\n"
         "              <tr bgcolor=\"#D4E4F3\"><td>Storage time (h)</td><td>" +
...
string get_colour(char category) {
  int opacity = convert_to_int(good_colour[0] +
                               (((category - 'A') / 4.0) * (bad_colour[0] - good_colour[0])));
  int blue = convert_to_int(good_colour[1] +
                            (((category - 'A') / 4.0) * (bad_colour[1] - good_colour[1])));
  int green = convert_to_int(good_colour[2] +
                             (((category - 'A') / 4.0) * (bad_colour[2] - good_colour[2])));
  int red = convert_to_int(good_colour[3] +
                           (((category - 'A') / 4.0) * (bad_colour[3] - good_colour[3])));
...
string get_point_kml(Pair *pair, string coordinates) {
  return "      <Placemark>\n"
         ...
         "          <IconStyle>\n"
         "            <scale>" +
         get_scale(pair->category) +
         "</scale>\n"
         "            <color>" +
         get_colour(pair->category) +
         "</color>\n"
         "            <Icon><href>http://maps.google.com/mapfiles/kml/paddle/" +
         pair->category +
         ".png</href></Icon>\n"
         "          </IconStyle>\n"
         "        </Style>\n" +
...
void update_kml_holder(KML_Holder *kml_holder, Pair *pair, Pair_KML *pair_kml) {
  ...
  kml_holder->uppers.push_back(
      get_reservoir_kml(&pair->upper, upper_colour, pair_kml->upper, pair));
  if (!pair->upper.brownfield)
    kml_holder->upper_dams.push_back(get_dam_kml(&pair->upper, pair_kml->upper));
  if (!pair->lower.ocean)
    kml_holder->lowers.push_back(
        get_reservoir_kml(&pair->lower, lower_colour, pair_kml->lower, pair));
