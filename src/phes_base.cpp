...
double find_required_volume(int energy, int head) {
  return (((double)(energy)*J_GWh_conversion) /
          ((double)(head)*water_density * gravity * generation_efficiency * usable_volume *
           cubic_metres_GL_conversion));
}
...
Model<short> *read_DEM_with_borders(GridSquare sc, int border) {
  ...
  const int neighbors[9][4][2] = {
      //[(Tile coordinates) , (Tile base)		 		  , (Tile limit)
      //, (Tile offset)	 	       ]
      ...
      {{sc.lat + 1, sc.lon + 1},
       {0, 3600 + border},
       {border, 3600 + 2 * border},
       {border - 3601, border + 3600}},
      {{sc.lat, sc.lon + 1},
       {border - 1, 3600 + border},
       {3600 + border, 3600 + 2 * border},
       {border - 1, border + 3600}},
      {{sc.lat - 1, sc.lon + 1},
       {3600 + border, 3600 + border},
       {3600 + 2 * border, 3600 + 2 * border},
       {border + 3599, border + 3600}},
      {{sc.lat - 1, sc.lon},
       {3600 + border, border},
       {3600 + 2 * border, 3601 + border},
       {border + 3599, border}},
      {{sc.lat - 1, sc.lon - 1},
       {3600 + border, 0},
       {3600 + 2 * border, border},
       {border + 3599, border - 3600}},
      {{sc.lat, sc.lon - 1},
       {border - 1, 0},
       {3600 + border, border},
  ...
  for (int i = 0; i < 9; i++) {
    ...
    ArrayCoordinate tile_start =
        ArrayCoordinate_init(neighbors[i][1][0], neighbors[i][1][1], get_origin(gs, border));
    ArrayCoordinate tile_end =
        ArrayCoordinate_init(neighbors[i][2][0], neighbors[i][2][1], get_origin(gs, border));
    ArrayCoordinate tile_offset =
        ArrayCoordinate_init(neighbors[i][3][0], neighbors[i][3][1], get_origin(gs, border));
    try {
      Model<short> *DEM_temp = new Model<short>(
          file_storage_location + "input/DEMs/" + str(gs) + "_1arc_v3.tif", GDT_Int16);
      if (i == 0) {
        DEM = new Model<short>(DEM_temp->nrows() + 2 * border - 1,
                               DEM_temp->ncols() + 2 * border - 1, MODEL_SET_ZERO);
      ...
      if (display)
        fprintf(stderr, "Could not find file %s: %s\n",
                convert_string(file_storage_location + "input/DEMs/" + str(gs) + "_1arc_v3.tif"),
                strerror(errno));
      if (i == 0)
        throw(1);
    }
...
BigModel BigModel_init(GridSquare sc) {
  BigModel big_model;
  GridSquare neighbors[9] = {(GridSquare){sc.lat, sc.lon},     (GridSquare){sc.lat + 1, sc.lon - 1},
  ...
  for (int i = 0; i < 9; i++) {
    ...
    try {
      big_model.flow_directions[i] =
          new Model<char>(file_storage_location + "processing_files/flow_directions/" + str(gs) +
                              "_flow_directions.tif",
                          GDT_Byte);
    } catch (int e) {
      if (display)
...
void set_FOM(Pair *pair) {
  ...
  double power_house_cost =
      powerhouse_coeff * pow(MIN(power, 800), (power_exp)) / pow(head, head_exp);
  double tunnel_cost = ((power_slope_factor * MIN(power, 800) + slope_int) * pow(head, head_coeff) *
                        seperation * 1000) +
                       (power_offset * MIN(power, 800) + tunnel_fixed);
  double power_cost = 0.001 * (power_house_cost + tunnel_cost) / MIN(power, 800);
  double energy_cost = dam_cost * 1 /
                       (pair->water_rock * generation_efficiency * usable_volume * water_density *
                        gravity * pair->head) *
                       J_GWh_conversion / cubic_metres_GL_conversion;
  ...
  if (pair->lower.ocean) {
    ...
    power_cost =
        0.001 * ((power_house_cost + tunnel_cost) / MIN(power, 800) + marine_outlet_cost / power);
  ...
  pair->category = 'Z';
  uint i = 0;
  while (i < category_cutoffs.size() &&
         pair->FOM < category_cutoffs[i].power_cost +
                         pair->storage_time * category_cutoffs[i].storage_cost) {
...
string str(Test test) {
  return energy_capacity_to_string(test.energy_capacity) + "GWh_" + to_string(test.storage_time) +
         "h";
}
...
GeographicCoordinate get_origin(double latitude, double longitude, int border) {
  return GeographicCoordinate_init(FLOOR(latitude) + 1 + (border / 3600.0),
                                   FLOOR(longitude) - (border / 3600.0));
}

ExistingReservoir get_existing_reservoir(string name) {
  ExistingReservoir to_return;
  vector<ExistingReservoir> reservoirs = read_existing_reservoir_data(convert_string(
...
ExistingPit get_pit_details(string pitname) {
  ExistingPit pit;
  vector<ExistingPit> pits = read_existing_pit_data(convert_string(
      file_storage_location + "input/existing_reservoirs/" + existing_reservoirs_csv));
