...
bool check_reservoir(Reservoir &reservoir, Model<bool> *seen, vector<ArrayCoordinate> *used_points,
                     BigModel &big_model) {
  ...
  while (volume * (1 + 0.5 / reservoir.water_rock) < (1 - volume_accuracy) * req_volume ||
         volume * (1 + 0.5 / reservoir.water_rock) > (1 + volume_accuracy) * req_volume) {
    ...
    // printf("%d %d %d %d\n", reservoir_big_ac.row, reservoir_big_ac.col, reservoir.pour_point.row,
    // reservoir.pour_point.col);
    while (!q.empty()) {
      ...
      volume += (wall_height - (DEM->get(big_ac.row, big_ac.col) -
                                DEM->get(reservoir_big_ac.row, reservoir_big_ac.col))) *
                find_area(p) / 100;  // Minor optimization with coslat?
      // if(wall_height<0)
      // 	printf("%d %d %f %f %d %d %f\n", big_ac.row, big_ac.col, wall_height,
      // (wall_height-(DEM->get(big_ac.row,big_ac.col)-DEM->get(reservoir_big_ac.row,reservoir_big_ac.col)))*find_area(p)/100,
      // DEM->get(big_ac.row,big_ac.col), DEM->get(reservoir_big_ac.row,reservoir_big_ac.col),
      // find_area(p));
      for (uint d = 0; d < directions.size(); d++) {
        ...
        if (flow_directions->check_within(neighbor.row, neighbor.col) &&
            ...
            ((DEM->get(neighbor.row + offset.row, neighbor.col + offset.col) -
              DEM->get(reservoir_big_ac.row, reservoir_big_ac.col)) < wall_height)) {
          q.push(neighbor);
        }
      }
    }
    // printf("%f %f %d\n", volume, wall_height, DEM->get(reservoir_big_ac.row,
    // reservoir_big_ac.col));
...
bool check_pair(Pair &pair, Model<bool> *seen, BigModel &big_model) {
  ...
  if (!pair.lower.brownfield && !pair.lower.ocean &&
      !check_reservoir(pair.lower, seen, &used_points, big_model))
    return false;
...
int main(int nargs, char **argv) {
  ...
  pairs = read_rough_pair_data(convert_string(file_storage_location + "processing_files/pairs/" +
                                              fname + "_rough_pairs_data.csv"));

  mkdir(convert_string(file_storage_location + "processing_files/pretty_set_pairs"), 0777);
  FILE *csv_data_file =
      fopen(convert_string(file_storage_location + "processing_files/pretty_set_pairs/" + fname +
