  ...
  search_config.logger.debug("Read in " + to_string(reservoirs.size()) + " reservoirs");
	...
	for (uint i=0;i< reservoirs.size(); i++) {
		if(reservoirs[i].identifier==rs){
			...
			ofstream kml_file(convert_string(file_storage_location+"output/"+reservoirs[i].identifier+".kml"), ios::out);
			...
			for(int i = 0; i<9; i++)
				if(big_model.neighbors[i].lat == (int)(FLOOR(reservoirs[i].latitude)-EPS) && big_model.neighbors[i].lon == (int)(FLOOR(reservoirs[i].longitude)+EPS))
					flow_directions = big_model.flow_directions[i];

			reservoirs[i].pour_point = convert_coordinates(GeographicCoordinate_init(reservoirs[i].latitude, reservoirs[i].longitude), flow_directions->get_origin());
		    ...
		    ArrayCoordinate reservoir_big_ac = convert_coordinates(convert_coordinates(reservoirs[i].pour_point), DEM->get_origin());
		    ...
		    vector<ArrayCoordinate> reservoir_polygon = convert_to_polygon(full_cur_model, offset, reservoirs[i].pour_point, 1);
