...
int main() {
  ...
  for (GridSquare gs : tasklist) {
    ...
    int lat = gs.lat;
    int lon = gs.lon;
    SHPHandle SHP = SHPCreate(convert_string(file_storage_location + "input/shapefile_tiles/" +
                                             str(gs) + "_shapefile_tile.shp"),
                              SHPT_POLYGON);
    if (SHP == NULL) {
      printf("Unable to create:%s\n",
             convert_string(file_storage_location + "input/shapefile_tiles/" + str(gs) +
                            "_shapefile_tile.shp"));
      exit(1);
    }
    ...
    for (uint ipolygon = 0; ipolygon < relevant_polygons->get(lat + 90, lon + 180).size();
      ...
      for (int i = 0; i < nVertices; i++) {
        GeographicCoordinate temp =
            polygons[relevant_polygons->get(lat + 90, lon + 180)[ipolygon]][i];
        padfX[i] = temp.lon;
        padfY[i] = temp.lat;
      }
      psObject = SHPCreateObject(SHPT_POLYGON, -1, 0, panParts, NULL, nVertices, padfX, padfY,
                                 padfZ, padfM);
