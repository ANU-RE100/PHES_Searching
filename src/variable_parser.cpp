...
double
    min_reservoir_water_rock;  // Minimum reservoir water to rock ratio at optimal dam wall height
double min_max_dam_height;     // Minimum maximum dam height (m) (Before overlapping filters) to be
                               // considered a potential reservoir
...
double min_slope;     // Minimum slope based on interpolated nearest point seperation between two
                      // reservoirs
...
double
    max_head_variability;  // Maximum amount the head can vary during water transfer (Default 0.35)
  ...
  while (getline(in, line)) {
    ...
    if (getline(ss1, line, ';')) {
      ...
      if (variable == "good_colour") {
        vector<string> t = read_from_csv_file(value);
        good_colour[0] = stoi(t[0]);
        good_colour[1] = stoi(t[1]);
        good_colour[2] = stoi(t[2]);
        good_colour[3] = stoi(t[3]);
      }
      if (variable == "bad_colour") {
        vector<string> t = read_from_csv_file(value);
        bad_colour[0] = stoi(t[0]);
        bad_colour[1] = stoi(t[1]);
        bad_colour[2] = stoi(t[2]);
        bad_colour[3] = stoi(t[3]);
      }
